...
# = importlib.machinery.SourceFileLoader("test", "test.pyc")

test = importlib.machinery.SourceFileLoader("test", "test.pyc").load_module()
...
class pj1_P1A(unittest.TestCase):
    ...
    def test_unimplemented_methods(self):
        ...
        room = pji.RectangularRoom(2,2,1)
...
class pj1_P1B(unittest.TestCase):
    ...
    def test_getset_robot_direction(self):
        ...
        robots = [pj1.Robot(solution_room, 1.0, 1) for i in range(4)]
...
class pj1_P2_ER(unittest.TestCase):
    ...
    def test_get_random_position(self):
        ...
        room = pj1.EmptyRoom(width, height, dirt_amount)
    ...
    def test_get_num_tiles(self):
        ...
        for i in range(10):        
            ...
            room_num_tiles = pj1.EmptyRoom(width, height, dirt_amount).get_num_tiles()
            sol_room_tiles = pj1.EmptyRoom(width, height, dirt_amount).get_num_tiles()
...
class pj1_P2_FR(unittest.TestCase):                  
    ...
    def test_is_tile_furnished(self):
        ...
        for trial in range(5):
            ...
            room = pj1.FurnishedRoom(width, height, dirt_amount)
    ...
    def test_get_num_tiles(self):
        ...
        for i in range(10):        
            ...
            sol_room = pj1.FurnishedRoom(width, height, dirt_amount)
            sol_room.furniture_tiles = room.furniture_tiles 
            # generate answers
...
class pj1_P3(unittest.TestCase):
    """This  tests EmptyRoom and Standard robot in various ways"""
    def createRoomAndRobots(self, num_robots):
        r = pj1.EmptyRoom(5, 7, 1)
        robots = [pj1.StandardRobot(r, 1.0, 1) for i in range(num_robots)]
        return r, robots
    ...
    def test_update_position_and_cleanStandardRobot(self):
        ...
        r = pj1.EmptyRoom(3, 5, 1)
        robot = pj1.StandardRobot(r, 1.0, 1)
        ...
        self.assertTrue(robotPos.get_x() == correctPos.get_x() and robotPos.get_y() == correctPos.get_y(),
                          ...
                          (pj1.Position(2.5, 2.5), robot.get_robot_position()))
...
"""The SIMULATION_TIME_LIMIT, SimulationThread class and SimulationTester class
    ...
    furniture is set up randomly. In future project it may make sense to manually
...
class SimulationThread(threading.Thread):
    ...
    def run(self):
        try:
            self.result = pj1.run_simulation(*self.args)
        except Exception:
...
class SimulationTester(unittest.TestCase):
    def run_simulation(self, bounds, parameters):
        """
        Tests pj1.run_simulation.  The number of time-steps
        pj1.run_simulation takes must fall between any of
        the (LOWER, UPPER) bound tuples inside bounds.

        pj1.run_simulation must also finish within a timelimit
        ...
        parameters: parameters to be passed into pj1.run_simulation
...
class pj1_P5_Standard(SimulationTester):
    ...
    def testSimulation1(self):
        ...
        try:
            self.run_simulation(((142, 173),), (1, 1.0, 1, 5, 5, 1, 1.0, 100, pj1.StandardRobot))
        except:
            print ("Unexpected error:", sys.exc_info()[1])
            raise
    def testSimulation2(self):
        "Test cleaning 75% of a 10x10 room (Standard Robot)"
        self.run_simulation(((183, 198),), (1, 1.0, 1, 10, 10, 1, 0.75, 100, pj1.StandardRobot))
    def testSimulation3(self):
        "Test cleaning 90% of a 10x10 room (Standard Robot)"
        self.run_simulation(((298, 327),), (1, 1.0, 1, 10, 10, 1, 0.9, 100, pj1.StandardRobot))
    def testSimulation4(self):
        "Test multiple robots (95% of a 20x20 room with 5 robots (Standard Robot))"
        self.run_simulation(((289, 303),), (5, 1.0, 1, 20, 20, 1, 0.95, 100, pj1.StandardRobot))
    def testSimulation5(self):
        "Test different speeds (90% of a 5x20 room with a robot of speed 0.2 (Standard Robot))"
        self.run_simulation(((1095, 1228),), (1, 0.2, 1, 5, 20, 1, 0.9, 100, pj1.StandardRobot))
    def testSimulation6(self):
        ...
        self.run_simulation(((155, 180),), (3, 0.5, 1, 10, 10, 1, 0.9, 100, pj1.StandardRobot))
    # new tests below here
    def testSimulation7(self):
         "Test cleaning 100% of a 5x5 room (Standard Robot, 5 dirt/tile, capcity = 3)"
         self.run_simulation(((206, 266),(180, 240)), (1, 1.0, 3, 5, 5, 5, 1.0, 100, pj1.StandardRobot))
    def testSimulation8(self):
        "Test cleaning 100% of a 5x5 room (Standard Robot, 6 dirt/tile, capacity = 3)"
        self.run_simulation(((206, 266),(180, 240)), (1, 1.0, 3, 5, 5, 6, 1.0, 100, pj1.StandardRobot))
    def testSimulation9(self):
        ...
        self.run_simulation(((387, 447),(384, 444)), (1, 0.2, 2, 3, 10, 6, 0.9, 100, pj1.StandardRobot))
    def testSimulation10(self):
        "Test multiple robots (95% of a 10x10 room with 5 robots (Standard Robot)) capacity = 2, 6 dirt/tile"
        self.run_simulation(((137, 198),(130, 190)), (5, 1.0, 2, 10, 10, 6, 0.95, 100, pj1.StandardRobot))
    def testSimulation11(self):
        ...
        self.run_simulation(((48, 108), (45, 104)), (3, 0.5, 2, 5, 5, 6, 0.9, 100, pj1.StandardRobot))

class pj1_P5_Faulty(SimulationTester):
    """test the simulation time cleaning the EmptyRoom with a FaultyRobot"""
    def testSimulation1(self):
        "Test cleaning 100% of a 5x5 room with FaultyRobot"
        x = pj1.run_simulation(1, 1.0, 1, 5, 5, 1, 1.0, 100, pj1.FaultyRobot)
        self.assertTrue(125 <= x <= 275,
                        "Simulation output was outside of 99.7% confidence interval!\n")    
    def testSimulation2(self):
        "Test cleaning 75% of a 10x10 room with FaultyRobot"
        x = pj1.run_simulation(1, 1.0, 1, 10, 10, 1, 0.75, 100, pj1.FaultyRobot)
    ...
    def testSimulation3(self):
        ...
        x = pj1.run_simulation(1, 1.0, 1, 10, 10, 1, 0.9, 100, pj1.FaultyRobot)
        self.assertTrue(360 <= x <=415, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
    def testSimulation4(self):
        "Test cleaning 100% of a 5x5 room with FaultyRobot"
        x = pj1.run_simulation(2, 1.0, 2, 5, 5, 5, 1.0, 100, pj1.FaultyRobot)
        self.assertTrue(160 <= x <=200, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
    def testSimulation5(self):
        "Test cleaning 75% of a 10x10 room with FaultyRobot"
        x = pj1.run_simulation(4, 1.0, 3, 10, 10, 5, 0.75, 100, pj1.FaultyRobot)
        self.assertTrue(101 <= x <=116, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")
    def testSimulation6(self):
        "Test cleaning 90% of a 10x10 room with FaultyRobot"
        x = pj1.run_simulation(5, 1.0, 3, 10, 10, 10, 0.9, 100, pj1.FaultyRobot)
        self.assertTrue(205 <= x <=225, "Simulation output was outside of 99.7% confidence interval! Took " + str(x) + " steps\n")

if __name__ == "__main__":
    suite = unittest.TestSuite()
    suite.addTest(unittest.makeSuite(pj1_P1A))
    suite.addTest(unittest.makeSuite(pj1_P1B))
    suite.addTest(unittest.makeSuite(pj1_P2_ER))
    suite.addTest(unittest.makeSuite(pj1_P2_FR))
    suite.addTest(unittest.makeSuite(pj1_P3))
    suite.addTest(unittest.makeSuite(pj1_P5_Standard))
    suite.addTest(unittest.makeSuite(pj1_P5_Faulty))
